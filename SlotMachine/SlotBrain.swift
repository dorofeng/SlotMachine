...
class SlotBrain {
    class func unpackSlotIntoSlotRows (slots: [[Slot]]) -> [[Slot]] { //reorganizing columns into rows
        var slotRow: [Slot] = []
        var slotRow2: [Slot] = []
        var slotRow3: [Slot] = []
    ...
    class func checkThreeInARow (slotRow: [Slot]) -> Bool {
        ...
        if slot1.value == slot2.value - 1 && slot1.value == slot3.value - 2 { //is there a descending 3 in a row
            return true
        }
        else if slot1.value == slot2.value + 1 && slot1.value == slot3.value + 2 { //is there a ascending 3 in a row
            return true
        }
        else {
            return false
