...
class Factory {
    //factories are a level of abstraction
    // class function differes from instance functions-- class you call on a class, instance you call on an instance
    class func createSlots() -> [[Slot]] {
        
        let kNumberofSlots = 3
        let kNumberofContainers = 3
        var slots: [[Slot]] = [] //create variable that's an array, that is going to hold arrays
        //slots = [ [slot1, slot2], [], [], [] ]
        
        for var containerNumber = 0; containerNumber < kNumberofContainers; ++containerNumber {
            var slotArray:[Slot] = []
            for var slotNumber = 0; slotNumber < kNumberofSlots; ++slotNumber {
                var slot = Factory.createSlot(slotArray) //can also remove Factory
